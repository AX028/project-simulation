...
def world_state_payload(world: WorldState) -> dict[str, Any]:
    """Return the version-independent field mapping for a world."""
    return {
...
def world_state_from_payload(raw: dict[str, Any]) -> WorldState:
    """Rebuild a world from the mapping produced by ``world_state_payload``."""
    settlements = {
        key: SettlementState(
            settlement_id=value["settlement_id"],
            name=value["name"],
    ...
    factions = {
        key: FactionState(
            ...
            relations={
                str(k): float(v)
                for k, v in value["relations"].items()
            },
            institutional_memory={
                str(k): float(v)
                for k, v in value["institutional_memory"].items()
            },
        )
...
class WorldStateStore:
    ...
    def read(path: str | Path) -> WorldState:
        ...
        try:
            return world_state_from_payload(raw)
